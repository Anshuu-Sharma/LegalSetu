...
import AnalysisModal from './Document/AnalysisModal';
...
const DocumentUpload: React.FC = () => {
  ...
  const currentFullAnalysis =
    documents.find((doc) => doc.id === analysisView)?.analysis;
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Upload documents and get AI-powered insights.</p>
        </div>
        <div className="grid lg:grid-cols-3 gap-8">
          <div>
            <LanguageSelector selected={selectedLanguage} onChange={setSelectedLanguage} />
            <FileUploader onFiles={handleFiles} dragActive={dragActive} setDragActive={setDragActive} />
          </div>
          <div className="lg:col-span-2">
            <DocumentList
              documents={documents.map((doc) => ({ ...doc, analysis: doc.analysis?.summary }))}
              onDelete={(id) => setDocuments((prev) => prev.filter((d) => d.id !== id))}
              onViewAnalysis={(id) => setAnalysisView(id)}
            />
          </div>
        </div>
        {currentFullAnalysis && <AnalysisModal analysis={currentFullAnalysis} onClose={() => setAnalysisView(null)} />}
      </div>
    </div>
  );
};
